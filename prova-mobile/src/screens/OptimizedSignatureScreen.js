...
const OptimizedSignatureScreen = () => {
  ...
  // Renderiza a assinatura como linhas conectadas
  const renderSignature = () => {
    if (signaturePoints.length === 0) return null;
    
    console.log('Renderizando assinatura com', signaturePoints.length, 'pontos');
    ...
    return signaturePoints.map((point, index) => {
      ...
      if (distance < 1) return null; // Pula pontos muito próximos
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        {signaturePoints.length > 0 && (
          <Text style={[styles.debugText, { color: '#00AA00', fontWeight: 'bold' }]}>
            ✅ Assinatura detectada! {signaturePoints.length} pontos capturados.
          </Text>
        )}
      </View>
...
export default OptimizedSignatureScreen;
